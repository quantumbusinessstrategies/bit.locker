...
class SQLiteStore:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                (
                    ...
                    float(score.get("upfront_cost_usd") or 0),
    ...
    def save_queue_item(
        ...
        with self._connect() as conn:
            try:
                ...
                queue_id, created = int(cursor.lastrowid), True
                conn.execute(
    ...
    def committed_paid_spend_usd(self) -> float:
        """Sum of upfront_cost_usd for claim_queue items not yet rejected/dead-ended.

        Used to enforce the owner's hard spend cap - any active (non-rejected) item
        requiring upfront payment counts against the cap, even before it reaches
        final approval, so the queue itself never outgrows the budget.
        """
        with self._connect() as conn:
            row = conn.execute(
                """
