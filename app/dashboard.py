...
from sources.custom_queries import load_custom_scan_queries, save_custom_scan_queries  # noqa: E402
...
def _resolve_database_path() -> Path:
    ...
    if not configured:
        return ROOT_DIR / "data" / "gain_entity.sqlite3"
...
DESKTOP_WALLPAPERS = {
    "Lofi Sunset": "linear-gradient(180deg,#2b1055 0%,#7c3f6b 35%,#e8735a 62%,#f4b860 78%,#f4b860 100%)",
...
DESKTOP_FONTS = {
    ...
    "Pixel (Press Start 2P)": '"Press Start 2P","Courier New",monospace',
    "Classic Desktop (Tahoma)": 'Tahoma,"Segoe UI",Verdana,sans-serif',
}
DEFAULT_DESKTOP_PREFS = {
    "wallpaper": "Lofi Sunset",
    "accent": "#000080",
    "font": "Retro Terminal (VT323)",
    "icon_density": "Normal",
    "icon_scale": 1.2,
...
def _is_live_ops_route() -> bool:
    try:
        return str(st.query_params.get("page") or "").lower() in {"live_ops", "liveops", "live"}
    except Exception:  # noqa: BLE001
        return False
...
def save_cancel_reminders(reminders: dict[str, dict[str, str]]) -> None:
    path = _cancel_reminders_path()
...
def inject_win95_desktop_style(prefs: dict[str, object]) -> None:
    ...
    st.markdown(
        ...
        div[class*="st-key-liveops_icon_"] button {{
            ...
            border-radius: 5px !important; box-shadow: none !important; color: #fff !important;
            font-family: {font_stack} !important; font-size: {icon_font_size} !important;
            text-shadow: 1px 1px 2px rgba(0,0,0,1), 0 0 5px rgba(0,0,0,0.95), 0 0 9px rgba(0,0,0,0.8) !important;
            white-space: pre-line !important; line-height: 1.25 !important;
        }}
        div[class*="st-key-liveops_icon_"] button:hover {{
            background: rgba(0,0,0,0.25) !important;
            border: 1px dotted rgba(255,255,255,0.5) !important;
        }}
        .win95-rail-label {{
            color: #fff; font-family: {font_stack}; font-weight: 900; font-size: 15px;
            text-shadow: 1px 1px 3px rgba(0,0,0,0.9); margin-bottom: 6px;
        }}
        .win95-titlebar {{
        ...
        div[class*="st-key-quicklaunch_"] button {{
            ...
            border: 1px solid #fff !important; border-right-color: #808080 !important; border-bottom-color: #808080 !important;
            border-radius: 0 !important; font-size: 18px !important; height: 32px !important;
            padding: 0 !important; min-width: 32px !important;
        }}
        div[class*="st-key-quicklaunch_"] button:hover {{
            background: #eaeaea !important;
        }}
        .win95-tray {{
            display: flex; align-items: center; justify-content: flex-end; gap: 6px;
            background: #dcdcdc; border: 1px solid #808080; border-right-color: #fff; border-bottom-color: #fff;
            padding: 4px 8px; font-size: 12px; font-weight: 700; font-family: {font_stack};
            height: 32px; box-sizing: border-box;
        }}
        .win95-tray span {{ padding: 2px 7px; border-radius: 3px; color: #fff; white-space: nowrap; }}
        div[class*="st-key-startmenu_"] button {{
            background: transparent !important; color: #000 !important; text-align: left !important;
            border: none !important; border-radius: 0 !important; font-family: {font_stack} !important;
            font-size: 15px !important; justify-content: flex-start !important; padding: 6px 10px !important;
        }}
        div[class*="st-key-startmenu_"] button:hover {{ background: {accent} !important; color: #fff !important; }}
        .win95-clock {{ font-family: {font_stack}; font-weight: 700; font-size: 13px; }}
        .win95-console {{
            background: #000; color: #33ff33; font-family: "VT323","Courier New",monospace;
...
def show_live_ops_route() -> None:
    ...
    is_live_mode = str(control.get("execution_mode") or "Dry Run") != "Dry Run"
    mode_color = "#1fbf4d" if is_live_mode else "#d1273d"
    mode_label = "🟢 LIVE" if is_live_mode else "🔴 DRY RUN"
    paid_mode_on = bool(control.get("paid_mode_enabled"))
    max_spend = float(control.get("max_spend_usd") or 0)

    prefs = load_desktop_preferences()
    inject_win95_desktop_style(prefs)

    if "liveops_open_app" not in st.session_state:
    ...
    with window_body, connect() as conn:
        ...
        now_ts = time.time()
        if not last_auto_ts or (now_ts - float(last_auto_ts)) > 600:
            st.session_state["liveops_last_auto_pass"] = now_ts
        ...
        if open_app == "desktop":
            ...
            st.caption(
                "The whole point of this tool is finding FREE gains. Paid mode is off by default on purpose - "
                "only turn it on if you specifically want to see opportunities that cost money upfront "
                "(like liquidation pallets), and only up to a hard dollar limit you set yourself. "
                "This cap is enforced by the discovery/scoring pipeline itself, not just the dashboard: "
                "opportunities requiring payment are blocked from ever reaching your queue once committed "
                "spend would cross the limit below."
            )
            committed_spend_row = conn.execute(
                """
                SELECT COALESCE(SUM(upfront_cost_usd), 0) AS total
                FROM claim_queue
                WHERE upfront_payment_required = 1
                  AND status NOT IN ('Rejected', 'Dead End', 'Later')
                """
            ).fetchone()
            committed_spend = float(dict(committed_spend_row).get("total") or 0)
            if paid_mode_on:
                st.metric(
                    "Committed paid spend",
            ...
            st.markdown("---")
            st.caption(
                "The button above advances items ALREADY in your queue. To go find brand new gains "
                "on the internet right now (instead of waiting for the scheduled background pass), use "
                "🔎 Add Scan on the desktop."
            )

        elif open_app == "custom_scan":
            st.caption(
                "This runs real internet search queries through the same discovery engine as the "
                "default scan - it does not replace the built-in broad scanning (grants, refunds, "
            ...
            if st.button("➕ Add to My Scans", type="primary") and new_query.strip():
                if new_query.strip() not in existing_custom_queries:
                    ...
                    st.info("That's already in your list.")

            if existing_custom_queries:
                st.markdown("**Your personalized scans**")
                for idx, query_text in enumerate(existing_custom_queries):
                    q_col, remove_col = st.columns([5, 1])
            ...
            st.markdown("---")
            if st.button(
                "🔍 Run Full Internet Scan Now",
                type="primary",
                use_container_width=True,
                help="Runs real source discovery + AI scoring immediately instead of waiting for the schedule.",
            ):
                with st.spinner("Scanning the internet and scoring what it finds - this can take a minute..."):
                    scan_result = _run_broad_fetch_scan()
                if int(scan_result.get("returncode", 1)) == 0:
                    st.success("Scan complete. New results will show up in Gain Finder / Pipeline.")
                else:
                    st.error("Scan did not finish cleanly - see output below.")
                st.code(str(scan_result.get("output") or "")[-3000:], language="text")

        elif open_app == "reminders":
            st.markdown("### ⏰ Cancel-By Reminders")
            reminders = load_cancel_reminders()
            if not reminders:
                st.info("No active reminders.")
            else:
                today = datetime.now(timezone.utc).date()
                for claim_id_str, info in sorted(reminders.items(), key=lambda kv: kv[1].get("cancel_by", "")):
                    try:
                        cancel_by = datetime.fromisoformat(info["cancel_by"]).date()
                    except Exception:  # noqa: BLE001
                        cancel_by = today
                    overdue = cancel_by <= today
                    color = "#d1273d" if overdue else "#1a1a1a"
                    flag = "🔴 CANCEL NOW" if overdue else "🟡 upcoming"
                    col_a, col_b = st.columns([4, 1])
                    with col_a:
                        st.markdown(
                            f"<span style='color:{color};font-weight:900;'>{flag}</span> "
                            f"— {info.get('title', 'item')} — cancel by **{cancel_by.isoformat()}**"
                            + (f" — [go cancel it]({info['link']})" if info.get("link") else ""),
                            unsafe_allow_html=True,
                        )
                    with col_b:
                        if st.button("✅ I Cancelled This", key=f"cancel_done_{claim_id_str}"):
                            reminders.pop(claim_id_str, None)
                            save_cancel_reminders(reminders)
                            st.rerun()

        elif open_app == "pipeline":
            ...
            for stage_index, stage in enumerate(stage_order):
                ...
                with st.expander(f"📁 {stage}  ({len(rows)})", expanded=(stage_index == 0)):
                    if rows.empty:
                        st.write("Nothing here right now.")
                        continue
                    labels = {
                        int(r.id): f"${float(r.expected_value_usd or 0):,.0f} · {r.title[:70]}"
                        for r in rows.itertuples()
                    }
                    selected_id = st.selectbox(
                        "Item", options=list(labels.keys()), format_func=lambda i: labels[i],
                        key=f"liveops_select_{stage}",
                    )
                    details = dict(load_claim_detail(conn, selected_id) or {})
                    st.markdown(f"**Next step:** {details.get('exact_next_step') or 'Not prepared yet.'}")
                    official_link = details.get("official_link") or details.get("url")
                    if official_link:
                        st.markdown(f"**Official link:** [{official_link}]({official_link})")
                    action_col, button_col = st.columns([2, 1])
                    with action_col:
                        selected_action = st.selectbox(
                            "Set status to", list(STATUS_ACTIONS.keys()), key=f"liveops_action_{stage}",
                        )
                    with button_col:
            ...
            proc_cols[0].metric("Chaos worker processes", _chaos_worker_process_count())
            try:
                connector_sessions = len(live_connector_rows(ROOT_DIR))
            except Exception:  # noqa: BLE001
                connector_sessions = 0
            proc_cols[1].metric("Live connector sessions", connector_sessions)
            proc_cols[2].metric("Execution mode", str(control.get("execution_mode") or "Dry Run"))
            st.markdown("**Queue status (Processes)**")
            status_counts = pd.read_sql_query(
                "SELECT status, COUNT(*) AS count FROM claim_queue GROUP BY status ORDER BY count DESC",
                conn,
            )
            show_dataframe(status_counts)
            st.markdown("**Performance**")
            perf_cols = st.columns(3)
            total_opps = conn.execute("SELECT COUNT(*) c FROM opportunities").fetchone()["c"]
            total_claims = conn.execute("SELECT COUNT(*) c FROM claim_queue").fetchone()["c"]
            ...
            st.caption(
                "Read-only system console — shows real automation log output. There is no arbitrary "
                "command execution here on purpose: a live shell exposed through a web dashboard is a "
                "security hole, so this console only runs the safe preset actions below."
            )
            log_text = _tail_text(ROOT_DIR / "data" / "chaos_worker.log", max_lines=60)
            st.markdown(
                f'<div class="win95-console">{html.escape(log_text or "No log output yet.")}</div>',
                unsafe_allow_html=True,
            )
            console_cols = st.columns(2)
            with console_cols[0]:
                if st.button("▶ run_automation_pass_now", use_container_width=True):
                    summary = run_completion_engine_pass(
                        conn,
                        load_effective_user_context(),
                        mode=str(control.get("execution_mode") or "Dry Run"),
            ...
            with console_cols[1]:
                if st.button("▶ refresh_queue_state", use_container_width=True):
                    store.normalize_required_inputs()
                    store.normalize_execution_state()
                    st.success("$ refresh_queue_state -> OK")
                    st.rerun()
            ...
            uploaded_wallpaper = st.file_uploader(
                "Upload a custom background image (select Custom Image above to use it)",
            ...
            font_choice = st.selectbox(
                ...
                index=list(DESKTOP_FONTS.keys()).index(prefs.get("font", "Retro Terminal (VT323)")),
            )
            accent_choice = st.color_picker("Accent color (title bars, Start menu)", value=str(prefs.get("accent", "#000080")))
            density_choice = st.selectbox(
                "Icon layout", ["Normal", "Compact"],
                index=["Normal", "Compact"].index(prefs.get("icon_density", "Normal")),
            )
            scale_choice = st.slider(
                "Icon & font scale",
                min_value=0.8,
                max_value=2.0,
                value=float(prefs.get("icon_scale", 1.2)),
                step=0.1,
                help="1.0 = original size. Applies to desktop icon labels and their emoji.",
            )
            save_col, reset_col = st.columns(2)
            with save_col:
                if st.button("💾 Save Personalization", type="primary", use_container_width=True):
                    new_prefs = {
                    ...
                    st.success("Saved. Reloading with your new look...")
                    st.rerun()
            with reset_col:
                if st.button("↺ Reset to Defaults", use_container_width=True):
                    save_desktop_preferences(dict(DEFAULT_DESKTOP_PREFS))
            ...
            if needs_you.empty:
                ...
                for row in needs_you.itertuples():
                    ...
                    with st.container(border=True):
                        ...
                        with top_col:
                            st.markdown(f"**${float(row.expected_value_usd or 0):,.0f} · {row.status} · {row.title[:80]}**")
                            st.caption(details.get("exact_next_step") or "Not prepared yet.")
                            official_link = details.get("official_link") or details.get("url")
                            if official_link:
                                st.caption(f"[{official_link}]({official_link})")
                        with action_col:
    ...
    mode_chip_text = f"💳 PAID (max ${max_spend:,.0f})" if paid_mode_on else "🆓 FREE"
    ...
    with taskbar:
        ...
        with taskbar_cols[0]:
            with st.popover("⊞ Start", use_container_width=True):
                for key, emoji, label in icon_defs:
                    menu_label = f"{emoji}  {label.replace(chr(10), ' ')}"
                    if st.button(menu_label, key=f"startmenu_{key}", use_container_width=True):
                        st.session_state["liveops_open_app"] = key
                        st.rerun()
                st.markdown(
                    '<a target="_self" href="/" style="display:block;padding:6px 10px;color:#000;'
                    'text-decoration:none;font-size:15px;">⟵  Classic View</a>',
                    unsafe_allow_html=True,
                )
                st.markdown("---")
                signed_in_user = str(st.session_state.get("owner_access_user") or "owner")
                st.caption(f"Signed in: {signed_in_user}")
        ...
        with taskbar_cols[2]:
            st.markdown(
                f'<div class="win95-tray">'
                f'<span style="background:{mode_chip_color};">{mode_chip_text}</span>'
                f'<span style="background:{mode_color};">{mode_label}</span>'
                f'<span class="win95-clock" style="background:transparent;color:#000;">{now_str}</span>'
                f"</div>",
                unsafe_allow_html=True,
            )
    ...
    components.html(
        ...
        <button
          style="
            ...
            cursor:pointer;
          "
          onclick="window.top.location.href='/?view=opportunities&sort=value#active-operator-table';"
...
def process_operator_action(conn: sqlite3.Connection) -> None:
    ...
    elif action == "toggle_execution_mode":
        control = load_autorun_control()
        current = str(control.get("execution_mode") or "Dry Run")
        control["execution_mode"] = (
            "Dry Run" if current != "Dry Run" else "Live Submit With Final Approval"
        )
        save_autorun_control(control)
        result = {
            "label": "Execution mode",
            "message": f"Switched to {control['execution_mode']}.",
            "status": "ok",
        }
...
def require_owner_access() -> bool:
    ...
    if configured_account_lookup:
        ...
        st.markdown(
            ...
            <div class="bios-screen">
              ...
              <div class="bios-line bios-cursor">AWAITING CREDENTIALS TO BOOT</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        <div class="bios-screen">
          ...
          <div class="bios-line">MEMORY TEST ......... OK</div>
          <div class="bios-line">ACCESS CONTROL ...... OK</div>
          <div class="bios-line bios-cursor">ENTER OWNER PIN TO BOOT</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _route_into_desktop_after_login() -> None:
    try:
...
def main() -> None:
    ...
    if _is_live_ops_route():
        show_live_ops_route()
        return
    if not _is_live_ops_route():
        st.markdown(
            """
            <a target="_self" href="/?page=live_ops" style="display:block;width:100%;
            background:linear-gradient(90deg,#ff2fa0,#ffd60a,#00e0ff);color:#000;
            text-align:center;font-weight:900;font-size:24px;padding:16px 0;
            text-decoration:none;letter-spacing:2px;border-bottom:6px solid #000;
            margin:-1rem -1rem 1rem -1rem;">
            🐸💰 ENTER LIVE OPS — NET REAL GAINS NOW 💰🐸
            </a>
            """,
            unsafe_allow_html=True,
        )
    ...
    if _is_vault_route():
        ...
        return
    if not _query_param("page", "") and not _query_param("view", ""):
        show_easy_asset_acquisition_route()
        return
...
def inject_bios_boot_style() -> None:
    st.markdown(
        """
        <style>
        ...
        @keyframes bios-blink { 50% { opacity: 0; } }
        .bios-screen p, .bios-screen span, .bios-screen div,
        [data-testid="stCaptionContainer"], label, .stMarkdown {
            color: #33ff33 !important;
            font-family: "VT323", "Consolas", "Courier New", monospace !important;
        }
        .stTextInput input {
...
def show_top_dashboard_metrics(conn: sqlite3.Connection) -> None:
    ...
    is_live_mode = str(control.get("execution_mode") or "Dry Run") != "Dry Run"
    mode_toggle_color = "#1fbf4d" if is_live_mode else "#d1273d"
    mode_toggle_label = "🟢 LIVE" if is_live_mode else "🔴 DRY RUN"
    mode_toggle_html = (
        f'<a target="_self" href="/?operator_action=toggle_execution_mode" '
        f'style="display:inline-block;padding:10px 22px;border-radius:999px;'
        f'background:{mode_toggle_color};color:#fff;font-weight:900;font-size:16px;'
        f'text-decoration:none;letter-spacing:1px;border:3px solid #fff;'
        f'box-shadow:0 0 0 2px {mode_toggle_color};">{mode_toggle_label}</a>'
    )
            ...
            <div>
              ...
              <div class="retro-content-header">
                ...
                <div style="text-align:right;">
                  <div style="margin-bottom:10px;">{mode_toggle_html}</div>
...
def load_autorun_control() -> dict[str, object]:
    ...
    if isinstance(payload, dict):
        ...
        control["paid_mode_enabled"] = bool(payload.get("paid_mode_enabled"))
        control["max_spend_usd"] = float(payload.get("max_spend_usd") or 0)
    return control
...
def load_desktop_preferences() -> dict[str, object]:
    ...
    if prefs.get("wallpaper") not in DESKTOP_WALLPAPERS and prefs.get("wallpaper") != "Custom Image":
        prefs["wallpaper"] = DEFAULT_DESKTOP_PREFS["wallpaper"]
    if prefs.get("font") not in DESKTOP_FONTS:
        prefs["font"] = DEFAULT_DESKTOP_PREFS["font"]
    try:
        prefs["icon_scale"] = max(0.8, min(2.0, float(prefs.get("icon_scale") or 1.2)))
    except (TypeError, ValueError):
        prefs["icon_scale"] = 1.2
    return prefs
...
def show_simple_start_panel(conn: sqlite3.Connection) -> None:
    ...
    st.markdown(
        """
        <div class="easy-ready-panel">
          <div class="easy-ready-title">SIMPLE START</div>
          <div class="easy-note">
            Use the Easy Launchpad for the fastest path. Turn Autopilot on for safe local work,
            run a single cycle when you want fresh movement, and keep final approvals manual.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    metric_cols = st.columns(4)
    metric_cols[0].metric("Autopilot", "ON" if enabled else "OFF")
    metric_cols[1].metric("Mode", mode)
    metric_cols[2].metric("Execution", execution_mode)
    metric_cols[3].metric("Waiting Approval", int(status.get("waiting_approval", 0) or 0))

    action_cols = st.columns([1.2, 1.2, 1.2, 1.4])
    action_cols[0].link_button("Open Easy Launchpad", "/?page=easy", use_container_width=True)
    action_cols[1].link_button("Open Vault", "/?page=vault", use_container_width=True)
    action_cols[2].link_button("Open Live Ops", "/?page=live_ops", use_container_width=True)
    if action_cols[3].button("Run One Safe Cycle", key="home_safe_cycle", use_container_width=True):
        with st.spinner("Running one safe automation cycle..."):
            result = _run_safe_automation_cycle()
        st.session_state["home_last_cycle_result"] = result
        st.rerun()

    control_cols = st.columns([1, 1, 2])
    if control_cols[0].button("Enable Autopilot", key="home_enable_autopilot", use_container_width=True):
        control["enabled"] = True
        if execution_mode == "Dry Run":
            control["execution_mode"] = "Live Assist"
        save_autorun_control(control)
        st.session_state["home_autopilot_message"] = "Autopilot enabled."
        st.rerun()
    if control_cols[1].button("Disable Autopilot", key="home_disable_autopilot", use_container_width=True):
        control["enabled"] = False
        save_autorun_control(control)
        st.session_state["home_autopilot_message"] = "Autopilot disabled."
        st.rerun()
    control_cols[2].caption("Safe automation stays approval-gated for connectors, signing, shipping, payout, and final accept steps.")

    if message := st.session_state.pop("home_autopilot_message", None):
        st.success(str(message))

    last_cycle = st.session_state.get("home_last_cycle_result")
    if isinstance(last_cycle, dict):
        with st.expander("Last safe cycle", expanded=False):
            st.write(f"Return code: {last_cycle.get('returncode', 'n/a')}")
            output = str(last_cycle.get("output") or "")
            if output:
                st.code(output[-6000:], language="text")
...
def show_dataframe(df: pd.DataFrame) -> None:
    if df.empty:
        st.write("None yet.")
        return
    link_column_names = {"official_link", "url", "link", "claim_url", "source_url"}
    column_config: dict[str, Any] = {}
    for col in df.columns:
        if col.lower() in link_column_names:
            column_config[col] = st.column_config.LinkColumn(
                col.replace("_", " ").title(),
                display_text="Open ↗",
            )
    st.dataframe(df, width="stretch", hide_index=True, column_config=column_config or None)
